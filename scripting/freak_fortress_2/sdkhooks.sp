...
float		g_DamageScale[MAXPLAYERS + 1] = { 1.0, ... };
...
*/
void BonkModifier(int entity)	 // Bonk Atomic Punch
{
	if (TF2_IsPlayerInCondition(entity, TFCond_Bonked) && !TF2_IsPlayerInCondition(entity, TFCond_SmallBlastResist))
	{
		PrintToChatAll("Detected Bonk Atomic Punch Effect");
		TF2_AddCondition(entity, TFCond_SmallBlastResist, 20.00);
	}
	if (TF2_IsPlayerInCondition(entity, TFCond_Dazed) && TF2_IsPlayerInCondition(entity, TFCond_SmallBlastResist))
	{
		TF2_RemoveCondition(entity, TFCond_Dazed);
		CreateTimer(0.2, BonkDuration, entity);
		PrintToChatAll("Replaced Slow & Extended duration");
	}
}

public Action BonkDuration(Handle timer, int entity)
{
	{
		TF2_AddCondition(entity, TFCond_Bonked, 8.00);	  // Bonk duration will not extend and flashes the effect for less then a second
		PrintToChatAll("Increased Bonk Duration");
	}
	return Plugin_Continue;
}

void ColaModifier(int entity)	 // Crit-a-Cola.
{
	if (TF2_IsPlayerInCondition(entity, TFCond_CritCola) && !TF2_IsPlayerInCondition(entity, TFCond_SmallBulletResist))
	{
		PrintToChatAll("Detected Crit-a-Cola Effect");
		CreateTimer(8.0, ColaDuration, entity);
		TF2_AddCondition(entity, TFCond_SmallBulletResist, 20.00);
	}
	if (TF2_IsPlayerInCondition(entity, TFCond_Dazed) && TF2_IsPlayerInCondition(entity, TFCond_SmallBlastResist))
	{
		TF2_RemoveCondition(entity, TFCond_Dazed);
		CreateTimer(0.2, BonkDuration, entity);
		PrintToChatAll("Extended Cola duration");
	}
}

public Action ColaDuration(Handle timer, int entity)
{
	{
		TF2_AddCondition(entity, TFCond_Bleeding, 88.00);	 // Cola duration will not extend will not extend and flashes the effect for less then a second like bonk
		PrintToChatAll("Increased Cola Duration");
	}
	return Plugin_Continue;
}
...
public Action OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	...
	// Probably inflictor here? Could actually be attacker and you need to get the owner netprop.
	if (!IsValidEntity(attacker))
	{
		TF2_AddCondition(attacker, TFCond_Milked);
		PrintToChatAll("something");
	}
	PrintToChatAll("idk");
	damage *= g_DamageScale[attacker];
